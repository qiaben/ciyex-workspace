...
import { PatientSnapshotDemoEditorInput, PatientChartEditorInput, EncounterFormEditorInput } from './ciyexEditorInput.js';
...
export class PatientSnapshotDemoEditor extends EditorPane {

	static readonly ID = 'workbench.editor.ciyexPatientSnapshotDemo';
	...
	constructor(
		...
		super(PatientSnapshotDemoEditor.ID, group, telemetryService, themeService, storageService);
	}
	...
	private _entityFields(entity: string): IEditFieldDef[] {
		...
		if (entity === 'encounters') {
			return withTypeaheadSearch(PatientSnapshotDemoEditor._encounterFormFields(), this.apiService);
		}
	...
	private static _encounterFormFields(): IEditFieldDef[] {
		return [
			...
			// --- Social History ---
			{
				key: 'sh_smoking', label: 'SH: Smoking', kind: 'select', widthPct: 33, options: [...SH_SMOKING_OPTIONS]
			},
			{
				key: 'sh_alcohol', label: 'SH: Alcohol', kind: 'select', widthPct: 33, options: [...SH_ALCOHOL_OPTIONS]
			},
			{
				key: 'sh_exercise', label: 'SH: Exercise', kind: 'select', widthPct: 33, options: [...SH_EXERCISE_OPTIONS]
			},
			{ key: 'sh_occupation', label: 'SH: Occupation', widthPct: 50 },
			{
				key: 'sh_drugs', label: 'SH: Recreational Drugs', kind: 'select', widthPct: 50, options: [...SH_DRUGS_OPTIONS]
			},
	...
	private _paginate<T>(key: string, items: T[]): { page: T[]; pageIdx: number; pageCount: number; total: number } {
		...
		const pageCount = Math.max(1, Math.ceil(total / PatientSnapshotDemoEditor.PAGE_SIZE));
		const pageIdx = Math.min(this._pageState.get(key) ?? 0, pageCount - 1);
		const start = pageIdx * PatientSnapshotDemoEditor.PAGE_SIZE;
		return { page: items.slice(start, start + PatientSnapshotDemoEditor.PAGE_SIZE), pageIdx, pageCount, total };
	}
	...
	private _renderPagerFooter(parent: HTMLElement, key: string, pageIdx: number, pageCount: number, total: number): void {
		...
		const from = pageIdx * PatientSnapshotDemoEditor.PAGE_SIZE + 1;
		const to = Math.min(from + PatientSnapshotDemoEditor.PAGE_SIZE - 1, total);
		// allow-any-unicode-next-line
	...
	override async setInput(input: PatientSnapshotDemoEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
	...
	private _openAddCardForm(): void {
		...
		openRecordEditDialog({
			...
			fields: [
				...
				{ key: 'billingCity', label: 'City', kind: 'text', widthPct: 50, typingPattern: '[A-Za-z ]', validationPattern: '^[A-Za-z ]+$', validationMessage: 'City may only contain letters and spaces.' },
				{ key: 'billingState', label: 'State', kind: 'text', widthPct: 50, typingPattern: '[A-Za-z ]', validationPattern: '^[A-Za-z ]+$', validationMessage: 'State may only contain letters and spaces.' },
